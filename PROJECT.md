...
- 地雷セルを開いた場合は、ゲームオーバー判定はまだ行わず、地雷タイル11を表示する。
...
- 盤面外は参照しない。
- 地雷セルは開かない。
- 1フレームで大量のBG更新が発生する可能性があるため、VBlank更新方針に従い、必要に応じて複数フレームに分割して表示更新する。
...
- 1マス分のBG表示更新をVBlankへ送るため、更新保留フラグ、タイル番号、BGアドレスをWRAMに保持する。
- 0連鎖オープン用のキュー領域をWRAMに最大81セル分保持する。
