...
- タイトル画面、通常プレイ、ポーズ、ゲームオーバー、ゲームクリアなどの画面・ゲーム状態を管理する状態変数の追加を検討する。
...
- [ ] UI・追加仕様
  - [ ] タイトル画面処理作成
    - [ ] 起動時にタイトル画面状態へ遷移する
    - [ ] タイトル画面でSTARTボタンを押すとゲーム本編へ遷移する
    - [ ] タイトル画面からゲーム本編へ遷移するときは新規ゲームとして初期化する
    - [ ] 初期実装ではタイトル画面に何も表示しなくてもよい
  - [ ] BG画像タイル変換ツール作成
    - [ ] 画像を8×8タイルへ分割する
    - [ ] 重複タイルを削除する
    - [ ] タイルセット画像を生成する
    - [ ] BGマップファイルを生成する
    - [ ] タイトルロゴだけでなく将来のBG画像にも使える汎用ツールとする
  - [ ] タイトル画面表示処理作成
    - [ ] タイトルロゴ画像を表示する
    - [ ] `PRESS START` を文字描画で表示する
    - [ ] `©2026 AKIHIRO SEKINE` を文字描画で表示する
  - [ ] ゲーム終了時のタイトル復帰
